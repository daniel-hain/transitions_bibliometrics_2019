...
plot_summary_timeline <- function(x, y1, y2, t, by, 
                                  y1_text = "Number", y2_text = "Share", 
                                  pal = NULL, select_cat = NULL,  
                                  t_min = NULL, t_max = NULL, label = FALSE){
  require(RColorBrewer)
  ...
  if(label == TRUE){
    ...
    p2 <- p2 +  geom_text(data = x %>% filter(!!t == max(!!t)) %>% arrange(desc(!!by)) %>% mutate(pos = cumsum(!!y2) - (!!y2 / 2)),
                           aes(x = max(!!t), y = pos, label =  !!by), size = 3) 
  }
